 ...
 * Executes as the single unified entrypoint for https://phone-whisper-server.pages.dev
 * Handles CORS preflights (204 No Content), reverse proxies all API/storage routes
 * to the live phone datacenter tunnel, and serves frontend static assets via env.ASSETS.
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // If this is a static frontend file, serve through Cloudflare Pages static assets
    if (!isApi) {
      if (env && env.ASSETS) {
